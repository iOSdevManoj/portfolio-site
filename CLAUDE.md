...
- **Mail templates** (`composeMail`, `section`, `enquiryBlocks`) build every prefilled mailto. They are written as business emails — greeting, grouped headings, a blank line under each question to type into, proper sign-off — because the body is the client's first impression. Spacing is one blank line inside a block, two between blocks; keep it consistent. `enquiryHref(topic)` puts the clicked tier or engagement on its own `Interested in:` line, **not** spliced into the sentence (that produced "would like to discuss ongoing engineering partner"). Cap the template at ~seven questions; longer ones get abandoned.
- **Verify mail changes by decoding them**, not by reading the source: fetch the page, extract `href="mailto:..."`, `urllib.parse` the query and print the body. Three defects — the grammar bug, uneven blank lines, and a booking line written from the wrong person's perspective — were invisible in the code and obvious in the rendered output.
